...
router.post("/", authMiddleware, validate(createBookingSchema), async (req, res) => {
    try {
        ...
        if (spaceId) {
            ...
            // Event-Driven: Dispara processamento de Analytics e Gamificação
            await dispatchEvent(backgroundQueue, 'BookingCreated', { 
                visitorId: userId, 
                providerId: tenantId, 
                value: 100 // Exemplo para pontuação
            });
        ...
        // Event-Driven: Notifica background (Emails, Gamification)
        await dispatchEvent(backgroundQueue, 'BookingCreated', { 
            visitorId: userId, 
            providerId: tenantId, 
            value: 0
